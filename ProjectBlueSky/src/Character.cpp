...
void Character::SetMove(const GameCtrl & ctl, weekListObj objList)
{
	...
	{
		if (animCnt == (6 * animTable["刺突砲"][ANIM_TBL_INV]))
		{
			if (dir == DIR_RIGHT)
			{
				AddObjList()(objList, std::make_unique<Shot>(pos + VECTOR2(256, 64), drawOffset, dir, padID));
...
void Character::Draw(void)
{
	...
	DrawBox(100, 65, 100 + (playerHPOld * 3), 85, 0xff0000, true);
	DrawBox(100, 65, 100 + (playerHP * 3), 85, hpColor, true);
	DrawGraph(70, 55, IMAGE_ID("image/ゲームシーン用/hpGage.png")[0], true);		// ﾌﾟﾚｲﾔｰ番号(2P)の描画
