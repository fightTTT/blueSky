...
void ResultScene::ResultDraw()
{
	...
	if (victoryFlag)
	{
		...
		imagePass[1] = ("image/" + charName[lpSceneMng.GetCharID(PAD_2)] + "/負け/"+"lose_" + std::to_string(animCnt / 8 % 2) + ".png");
	}
	else
	{
		imagePass[0] = ("image/" + charName[lpSceneMng.GetCharID(PAD_1)] + "/負け/"+"lose_" + std::to_string(animCnt / 8 % 2) + ".png");
